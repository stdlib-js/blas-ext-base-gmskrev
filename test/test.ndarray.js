...
tape( 'the function reverses a strided array according to a mask (accessors)', function test( t ) {
	...
	gmskrev( x.length, toAccessorArray( x ), 1, 0, toAccessorArray( mask ), 1, 0 );
	t.deepEqual( x, expected, 'returns expected value' );
...
tape( 'the function supports specifying a stride (accessors)', function test( t ) {
	...
	gmskrev( 3, toAccessorArray( x ), 2, 0, toAccessorArray( mask ), 1, 0 );
	t.deepEqual( x, expected, 'returns expected value' );
...
tape( 'the function supports specifying a negative stride (accessors)', function test( t ) {
	...
	x = [ 4.0, -3.0, -1.0, -5.0 ];
	mask = [ 0, 0 ];
	expected = [ 4.0, -5.0, -1.0, -3.0 ];

	gmskrev( 2, toAccessorArray( x ), -2, x.length-1, toAccessorArray( mask ), 1, 0 );
	t.deepEqual( x, expected, 'returns expected value' );
...
tape( 'the function excludes masked elements from the reversal (accessors)', function test( t ) {
	...
	gmskrev( 8, toAccessorArray( x ), 1, 0, toAccessorArray( mask ), 1, 0 );
	t.deepEqual( x, expected, 'returns expected value' );
...
tape( 'the function supports specifying a mask stride', function test( t ) {
	...
	gmskrev( x.length, x, 1, 0, mask, 2, 0 );
	t.deepEqual( x, expected, 'returns expected value' );
...
tape( 'the function supports specifying an offset', function test( t ) {
	...
	expected = [ 1.0, 5.0, 4.0, 3.0, 2.0 ];

	gmskrev( 4, x, 1, 1, mask, 1, 0 );
	t.deepEqual( x, expected, 'returns expected value' );
	t.end();
});
